...
function getAdminId(){
 return new Promise((resolve, reject) => {
      connectionDB.query(`SELECT usrId FROM names WHERE name='administrator'`,(err,rows)=>{
        if (err){
          reject(err);
        } 
        if (rows.length === 0) {
...
class UserRoute {
  ...
  async userById(id, table = 'users'){
    return new Promise((resolve,reject)=>{
      ...
      mysql.query(`SELECT name from ${table} NATURAL JOIN names WHERE usrId=${Number(id)};`,(err,rows)=>{
        if (err) {
          reject(err);
        }
        resolve(rows[0].name);
  ...
  try{
    ...
    //write into a 'names' table
    await   new Promise((resolve, reject) => {
      mysql.query(`INSERT INTO  names ( name, usrId) VALUES ?`, [dataToSendForNames], (err, rows) => {
        if (err) {
          reject({ status: 'fail', result: err.code })// ER_DUP_ENTRY - user is already exists
        }
        resolve(rows);
...
//if there are all rigt -commit changes in the tables
  ...
  async isUserInDB (name, tablename='names') {
    const mysql = this.data.get(this).connDB
    return new Promise((resolve, reject) => {
  ...
  async getUsr (name, tablename) {
    const mysql = this.data.get(this).connDB
    return new Promise((resolve, reject) => {
      mysql.query(`SELECT usrId, hashedPassword, status, fatt FROM ${tablename} NATURAL JOIN names WHERE name='${name}'`, (err, rows) => {
...
class AdminRoutes {
  ...
  async getUsers() {
    ...
    return new Promise((resolve, reject) => {
      dbc.query('SELECT usrId, name, status, fatt AS fail_login FROM users NATURAL JOIN names', (err, rows) => {
        if (err) {
          reject(err)
        } else {
          resolve(rows)
  ...
  if (await usersAuthProcedures.comparePassword(req.body.captcha, req.cookies.regCookie)) {
    ...
    try {
      ...
      if (e.result === 'ER_DUP_ENTRY') {
        ...
        await beginRegistration(req, res, 409, svgCaptcha, UserRouteInst, 'userID already exists!')
        return
      } else {
        res.status(500)
