...
function getAdminId(){
 return new Promise((resolve, reject) => {
      connectionDB.query(`SELECT usrId FROM users WHERE name='administrator'`,(err,rows)=>{
        if (err){
          reject(err);
        } 
        if (rows.length === 0) {
...
class UserRoute {
  ...
  async userById(id, table = 'users'){
    return new Promise((resolve,reject)=>{
      ...
      mysql.query(`SELECT name from ${table} WHERE usrId=${Number(id)};`,(err,rows)=>{
        if (err) {
          reject(err);
        }
        resolve(rows[0].name);
  ...
  async saveUsr (ud = { name: '',  hashedPassword: '' }, tablename) {
    const mysql = this.data.get(this).connDB
    const dataToSend = [[ud.name, ud.hashedPassword, Number(-1)|0]]
    return new Promise((resolve, reject) => {
      mysql.query(`INSERT INTO ${tablename} ( name, hashedPassword, sessionStatus) VALUES ?`, [dataToSend], (err, rows) => {
        if (err) {
  ...
  async getSessionStatus (usrId, tablename) {
    const mysql = this.data.get(this).connDB
    return new Promise((resolve, reject) => {
      mysql.query(`SELECT sessionStatus FROM ${tablename}  WHERE usrId=${usrId}`, (err, rows) => {
        if (err) {
          reject({ status: 'fail', result: err.code })
        }
        if (rows.length === 1) {
          resolve({ status: 'succ', result: rows[0].sessionStatus })
        } else {
          resolve({ status: 'fail' })
        }
      })
  ...
  async setSession (usrId, sessionStatus, tablename) {
    ...
    return new Promise((resolve, reject) => {
      ...
      mysql.query(`UPDATE  ${tablename} SET sessionStatus=?  WHERE usrId='${usrId}'`, myData, (err, rows) => {
        if (err) {
          reject({ status: 'fail', result: err.code })
        }
          resolve({ status: 'succ' })
  ...
  async getUsr (name, tablename) {
    const mysql = this.data.get(this).connDB
    return new Promise((resolve, reject) => {
      mysql.query(`SELECT name, usrId, hashedPassword, sessionStatus, fatt FROM ${tablename} WHERE name='${name}'`, (err, rows) => {
  ...
  * otherwie ersolve {ststys:'user exists'} */
  async pRegisterUser (x = { name: '', password: '',}) {
    ...
    return await this.saveUsr({ name: x.name,  hashedPassword: hashed }, 'users')
  }
  ...
  async pStartSession (x = { name: '', password: '' }, cryptoPassword) {
    ...
    this.setSession(usrData.data.usrId, Number(1), 'users')
...
class AdminRoutes {
  ...
  async getUsers() {
    ...
    return new Promise((resolve, reject) => {
      dbc.query('SELECT usrId, name, sessionStatus, fatt AS fail_login FROM users', (err, rows) => {
        if (err) {
          reject(err)
        } else {
          resolve(rows)
